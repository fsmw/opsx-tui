...
Define requirements for the Change browser screen (ChangesView), which lists active and archived changes with real-time search, a detail panel showing parsed artifacts (proposal, design, tasks, delta specs, diagnostics), and reactive refresh on workspace updates.
...
When a change is selected in the list, the detail panel SHALL show change name and state, proposal sections (if `parsed_proposal` exists), design decisions (if `parsed_design` exists), delta specs list (if `delta_specs` is non-empty), task list with progress bar (if `parsed_tasks` exists), and artifact diagnostics (if `artifact_diagnostics` is non-empty).
...
`ChangesView` SHALL support keyboard navigation: up/down arrows to move selection in the change list, Enter to select a change, `/` to focus the search input.
