...
Define requirements for the application shell — the main screen chrome including header, tab navigation, content area with six placeholder views, footer, help modal, and error modal.
...
The shell SHALL NOT import `project_screen.py`. Starting the app without a valid project SHALL call `self.exit()` with an error notification, not push an interactive screen.
